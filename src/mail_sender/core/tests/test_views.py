...
class SendEmailTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            username="ivan", password="ivan", email="ivan@ma.ru"
        )
        self.client = Client()
...
class EnableDisableTaskTest(TestCase):
    ...
    def setUp(self):
        ...
        self.authorized_client.force_login(self.user)
        self.interval = IntervalSchedule.objects.create(every="1", period="hour")
        self.periodic_task_enabled = PeriodicTask.objects.create(
            name="name", task="123", interval=self.interval, enabled=True
        )
        self.periodic_task_disabled = PeriodicTask.objects.create(
            name="name1", task="123", interval=self.interval, enabled=False
        )

    def test_disable_task_POST(self):
        redirect_url = reverse("tasks")
        response = self.authorized_client.post(
            path=reverse("enable-disable-task"),
            data={
...
class DeleteTaskTest(TestCase):
    ...
    def setUp(self):
        ...
        self.authorized_client = Client()
        self.authorized_client.force_login(self.user)
        self.email_data = EmailData.objects.create(
            emails={"0": "v@mail.ru"}, subject="subject1", message="message1"
        )
        self.interval = IntervalSchedule.objects.create(every="1", period="hour")
        self.first_periodic_task = PeriodicTask.objects.create(
            name="name", task="123", interval=self.interval, enabled=True
        )
        self.first_core_task = TaskCore.objects.create(
            user=self.user, email_data=self.email_data, task=self.first_periodic_task
        )

    def test_delete_periodic_and_core_task_POST(self):
        redirect_url = reverse("tasks")
        response = self.authorized_client.post(
            path=reverse("delete-task"),
            data={
...
class SignupTest(TestCase):
    ...
    def setUp(self):
        self.user1 = User.objects.create_user(
            username="user1", password="ivan", email="ivan@ma.ru"
        )
        ...
        self.user2 = User.objects.create_user(
            username="user2", password="ivan", email="ivan@ma.ru"
        )
...
class SigninTest(TestCase):
    ...
    def setUp(self):
        self.user1 = User.objects.create_user(
            username="user1", password="user1", email="ivan@ma.ru"
        )
        ...
        self.user2 = User.objects.create_user(
            username="user2", password="user2", email="ivan@ma.ru"
        )
