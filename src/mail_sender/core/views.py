...
class SendEmail(LoginRequiredMixin, View):
    ...
    def post(self, request: http.HttpRequest) -> http.HttpResponseRedirect:
        ...
        if form.is_valid():
            ...
            send_email_celery.delay(request.user.id, cd['emails_list'], cd['subject'], cd['message'])
    ...
    def post(self, request: http.HttpRequest) -> http.HttpResponseRedirect:
        ...
        if form.is_valid():
            ...
            periodic_task_obj = PeriodicTask.objects.create(
                ...
                args=json.dumps([request.user.id, cd['emails_list'], cd['subject'], cd['message']])
            )

            email_data = EmailData.objects.create(emails=cd['emails_list'], subject=cd['subject'], message=cd['message'])
...
class Tasks(LoginRequiredMixin, View):
    ...
    def get(self, request: http.HttpRequest) -> http.HttpResponse:
        tasks = TaskCore.objects.filter(user=request.user).order_by('-task__date_changed')
